...
export default function AccountSettings({id}: AccountSettingsProps) {
  ...
  const { mutate: deleteAccount, isPending } = useMutation<void, AxiosError<IError>, void>({
    mutationFn: async () => {
    ...
    onError: (error) => {
      toast.error(error.response?.data?.detail ?? error.message ?? "Failed to delete account.")
    }
  })
    ...
    <div className="relative">
      ...
      <button
        ...
        onClick={() => modalStore.openModal("destructive", {
          title: "Are you sure?",
          subtitle: "Once you delete your account it cannot be undone. You will also lose all data associated with your account.",
          button: {
